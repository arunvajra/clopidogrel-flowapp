...
def create_message_bubble(text, is_question=True, is_current=False, is_prompt=False):
    # Escape HTML special characters in the text
    escaped_text = html.escape(text)

    bubble_color = "#D6EAF8" if is_question else "#AED6F1"
    text_color = "#000000"
    align = "left" if is_question else "right"
    font_weight = "bold" if is_current or is_prompt else "normal"
    font_size = "1.2em" if is_current or is_prompt else "1em"
    markdown = f"""
    <style>
    .bubble {{
...
def show_step(step_id):
    ...
    if step_type == "question":
        ...
        create_message_bubble(question["label"], is_question=True, is_current=True)
        ...
        create_message_bubble(prompt["label"], is_question=False, is_prompt=True)
        st.session_state.next_step = None
        return

def main():
    st.set_page_config(page_title="Clopidogrel Flow App", layout="wide")
